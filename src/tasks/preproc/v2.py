...
def plot_raw_lightcurves(star_df, output_dir, is_folded=False):
    """
    ...
    for star_id in star_ids:
        ...
        if not good_data.empty:
            plt.errorbar(good_data['mjd'] % good_data['period'].iloc[0] if is_folded else good_data['mjd'], 
                         good_data['mag'], 
        ...
        if not bad_data.empty:
            plt.errorbar(bad_data['mjd'] % bad_data['period'].iloc[0] if is_folded else bad_data['mjd'], 
                         bad_data['mag'], 
...
def plot_processed_lightcurves(processed_df, output_dir):
    ...
    Args:
        ...
        output_dir: Directory to save plots
        is_folded: Whether to fold the light curves
    """
...
def main(args):
    ...
    if args.plot:
        ...
        plot_raw_lightcurves(few_stars, args.output_dir, is_folded=args.fold)
