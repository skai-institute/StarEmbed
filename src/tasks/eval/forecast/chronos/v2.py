...
def predict_lightcurve(processed_df, model_name="amazon/chronos-t5-small", context_percent=0.8, split_point=None, fig_dir=None):
    """
    ...
    Args:
        ...
        fig_dir: Directory to save the plots (default: None)
    Returns:
    ...
    for star_id in processed_df.index.get_level_values('ps1_objid').unique():
        ...
        # print(f"Predicting {star_id} with {len(context_tensor)} context and {len(target)} target")
        ...
        if fig_dir is not None:
            ...
            phases = star_data.index.get_level_values(star_data.index.names[0])
            ...
            plt.title(f'Star {star_id} (Class {star_data["star_class"].iloc[0]}) Prediction - MSE: {mse:.4f}')
            plt.xlabel('Phase')
...
def main(args):
    ...
    predictions = predict_lightcurve(
        ...
        fig_dir=f"{args.output_dir}/plots" if args.plot else None
    )
