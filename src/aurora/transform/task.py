...
class MaskedPrediction(MapFuncMixin, CheckArrNDimMixin, Transformation):
    ...
    def __call__(self, data_entry: dict[str, Any]) -> dict[str, Any]:
        # print(f"[wj debug] MaskedPrediction, file path: {__file__}")
        # print(f"[wj debug] data_entry: {data_entry.keys()}")
        # print(f"[wj debug] target_field: {self.target_field}")
        # print(f"[wj debug] truncate_fields: {self.truncate_fields}")
        # print(f"[wj debug] optional_truncate_fields: {self.optional_truncate_fields}")
        # print(f"[wj debug] prediction_mask_field: {self.prediction_mask_field}")
        # print(f"[wj debug] expected_ndim: {self.expected_ndim}")
        # print(f"[wj debug] min_mask_ratio: {self.min_mask_ratio}")
        # print(f"[wj debug] max_mask_ratio: {self.max_mask_ratio}")
    ...
    def _generate_prediction_mask(
        ...
        # print(f"[wj debug] _generate_prediction_mask, file path: {__file__}")
        # print(f"[wj debug] target shape: {target.shape}")
        self.check_ndim("target", target, self.expected_ndim)
        # print(f"[wj debug] target shape: {target.shape}")
        var, time = target.shape[:2]
        prediction_mask = np.zeros((var, time), dtype=bool)
        # print(f"[wj debug] prediction_mask shape: {prediction_mask.shape}")
        mask_ratio = np.random.uniform(self.min_mask_ratio, self.max_mask_ratio)
        # print(f"[wj debug] mask_ratio: {mask_ratio}")
        mask_length = max(1, round(time * mask_ratio))
        # print(f"[wj debug] mask_length: {mask_length}")
...
class ExtendMask(CheckArrNDimMixin, CollectFuncMixin, Transformation):
    ...
    def __call__(self, data_entry: dict[str, Any]) -> dict[str, Any]:
        # print(f"[wj debug] ExtendMask, file path: {__file__}")
        # print(f"[wj debug] data_entry: {data_entry.keys()}")
        # print(f"[wj debug] fields: {self.fields}")
        # print(f"[wj debug] optional_fields: {self.optional_fields}")
        # print(f"[wj debug] mask_field: {self.mask_field}")
        # print(f"[wj debug] expected_ndim: {self.expected_ndim}")
        target_mask: np.ndarray = data_entry[self.mask_field]
        # print(f"[wj debug] target_mask shape: {target_mask.shape}")
    ...
    def _generate_target_mask(
        ...
        # print(f"[wj debug] _generate_target_mask, file path: {__file__}")
        # print(f"[wj debug] data_entry: {data_entry.keys()}")
        ...
        # print(f"[wj debug] field_target_mask shape: {field_target_mask.shape}")
