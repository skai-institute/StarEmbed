...
#"/projects/p32795/weijian/embs/csdr1_raw4_catflags_filtered_embs_hand_crafted_trn_val_tst_bandgr",
)
...
def main(seed):
    ...
    for f in input_embs:
        ...
        for i in np.unique(np.asarray(y_val)):
            ...
            m = IsolationForest(n_estimators=200, random_state=seed)
            m.fit(X_train[idx, :])
            models[i] = m
