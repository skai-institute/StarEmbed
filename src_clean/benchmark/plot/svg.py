# Execute your v6+fixedticks script and ALSO save SVGs for each panel and the combined figure.
...
output_dir = "/projects/b1094/StarEmbed/src/output"
...
LEFT_RANK = 0.24; RIGHT_RANK = 1.00; BOTTOM = 0.12; TOP = 0.88
LEFT_CM = 0.18; RIGHT_CM = 0.98
GAP_12 = 30
GAP_23 = 2
...
def save_all(fig, stem):
    ...
    fig.savefig(f"{output_dir}/{stem}.png", dpi=DPI, bbox_inches="tight")
    fig.savefig(f"{output_dir}/{stem}.pdf", dpi=DPI, bbox_inches="tight")
    fig.savefig(f"{output_dir}/{stem}.svg", dpi=DPI, bbox_inches="tight")
    plt.close(fig)

# Rank panel with fixed integer ticks 1..7
rank_mat = to_rank(f1)
fig, ax = plt.subplots()
ax.grid(False)
...
for i in range(rank_mat.shape[0]):
    for j in range(rank_mat.shape[1]):
        r = rank_mat[i, j]
        color = "black" if r <= threshold else "white"
...
ticks = list(range(1, int(rank_mat.max()) + 1))
cbar.locator = FixedLocator(ticks)
cbar.formatter = FixedFormatter([str(t) for t in ticks])
cbar.update_ticks()
cbar.set_label("Rank (1 = best)", fontsize=CBAR_LABELSIZE)
cbar.ax.tick_params(labelsize=CBAR_LABELSIZE)

fig.subplots_adjust(left=LEFT_RANK, right=RIGHT_RANK, bottom=BOTTOM, top=TOP)
plt.tight_layout()
save_all(fig, "_rank_square_box_v6_with_fixedticks")
...
fig.subplots_adjust(left=LEFT_CM, right=RIGHT_CM, bottom=BOTTOM, top=TOP)
plt.tight_layout()
save_all(fig, "_cm_mid_v6_fixedticks")

# Right CM (no y-labels, colorbar kept)
fig, ax = plt.subplots()
ax.grid(False)
...
fig.subplots_adjust(left=LEFT_CM, right=RIGHT_CM, bottom=BOTTOM, top=TOP)
plt.tight_layout()
save_all(fig, "_cm_right_v6_fixedticks")

# Stitch (PNG) + make a vector (SVG) using matplotlib with the PNGs embedded
rank_png = f"{output_dir}/_rank_square_box_v6_with_fixedticks.png"
cm1_png  = f"{output_dir}/_cm_mid_v6_fixedticks.png"
cm2_png  = f"{output_dir}/_cm_right_v6_fixedticks.png"
...
combined_png = "/projects/b1094/StarEmbed/src/output/combined_rank_and_confusions_row_v6_fixedticks.png"
combined_pdf = "/projects/b1094/StarEmbed/src/output/combined_rank_and_confusions_row_v6_fixedticks.pdf"
canvas.save(combined_png)
canvas.save(combined_pdf)

# Make an SVG "combined" by laying out the PNGs in a figure.
import matplotlib.image as mpimg
ratio = total_w / h
fig, ax = plt.subplots(figsize=(ratio*6, 6))  # height ~6in, width scaled by ratio
ax.axis("off")
img = mpimg.imread(combined_png)
ax.imshow(img)
combined_svg = f"{output_dir}/combined_rank_and_confusions_row_v6_fixedticks.svg"
fig.savefig(combined_svg, bbox_inches="tight")
plt.close(fig)

print(f"Files saved:")
print(f"- Individual PNGs, PDFs, SVGs in: {output_dir}")
